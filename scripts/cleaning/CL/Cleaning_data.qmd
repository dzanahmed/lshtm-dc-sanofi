...
Epi calendar form 31/12/2021 to 31/12/2023
...
Data produce by Chilean Mistry of health and Science [GitHub](https://github.com/MinCiencia/Datos-COVID19).
...
The data set was taken from the same previous source. It is called "HospitalizadosEtario_Acumulado_std.csv". Number represent cumulative number of hospitalised people.

There were 2 measurement per week. I took the last measurement as the representative one. Then for getting the "new number (considering admissions and discharges)" I subtracted the prior week value
...
h1 <- hospilalised_week %>% 
  filter(year == 2022) %>% 
  ggplot(aes(week,cumul_cases, group = age_group, color = age_group)) +
  geom_line() 

  
h2 <-hospilalised_week %>% 
  filter(year == 2022) %>% 
  ggplot(aes(week,(daily/19000000)*100000, group = age_group, color = age_group)) +
  geom_line() 
...
Data extracted from Epe Deparment at [Minstry of Health]((http://epi.minsal.cl/visualizacion-interactiva-influenza/#/))

```{r warning = F, message = F}
sari_chile <- read_csv('https://raw.githubusercontent.com/IgnaLeiva/sanofii/main/doc/SARI_minsal.csv')
...
# set covid previos 2020 to cero

sari_chile$COVID[sari_chile$year != 2022 ] <- 0


# sari_chile %>% 
#   ggplot(aes(week, COVID, group = year, color = year)) +
#   geom_line()
...
```{r warning = F, message = F}
flunet_raw <- read_csv("https://raw.githubusercontent.com/IgnaLeiva/sanofii/main/doc/flunet_chile.csv")
...
#write_csv(chi_premerged_total, "/Users/igna/Desktop/DataSetChile/premerged_data/chi_premerged_total.csv")

chi_premerged_total %>% 
  ggplot(aes(week, cases_abs_covid19, group = year, color = year))+
  geom_line()

```
## The data set by age for hospitalizations
...
#write_csv(chi_premerged_by_age, "/Users/igna/Desktop/DataSetChile/premerged_data/chi_premerged_by_age.csv")

chi_premerged_by_age %>% 
  filter(year == 2022) %>% 
  ggplot(aes(week,hsp_abs_covid, group = age_group , color = age_group)) +
    geom_line() +
    ggtitle("ABsolute numbers")

chi_premerged_by_age %>% 
  filter(year == 2022) %>% 
  ggplot(aes(week,hsp_rate_covid19, group = age_group , color = age_group)) +
    geom_line() +
  ggtitle("Rate numbers")
```

## Australia
```{r}

flunet_raw <- read_csv("/Users/igna/Documents/LSHTM/Term 2/Data Challenge/lshtm-dc-sanofi/data/processed_data/AUS/australia_fluNet.csv")
...
#write_csv(aus_premerged_total, "/Users/igna/Documents/LSHTM/Term 2/Data Challenge/sanofii/doc/aus_premerged_total.csv")

```

## France non_sentinel
```{r}
preclean_fran_flu<- read_csv("/Users/igna/Documents/LSHTM/Term 2/Data Challenge/lshtm-dc-sanofi/data/processed_data/FRA/france_fluNet.csv")

preclean_fran_covid<- read_csv("/Users/igna/Documents/LSHTM/Term 2/Data Challenge/lshtm-dc-sanofi/data/processed_data/FRA/france_covid19.csv")
...
#write_csv(FR_premerged_total, "FR/FR_premerged_total.csv")
